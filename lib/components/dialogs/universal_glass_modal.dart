...
class UniversalGlassModal extends ConsumerWidget {
  const UniversalGlassModal({
    super.key,
    this.title, this.subtitle, this.leading, this.child,
    this.actions = const [],
    this.maxWidth = 720,
    this.maxHeight,
    this.minHeight = 0,
    this.headerSpacing = 16, this.contentSpacing = 18, this.actionsSpacing = 10,
    this.padding,
    this.scrollable = true, this.scrollController,
    this.borderRadius,
  ...
  static Future<T?> show<T>({
    ...
    String? title, String? subtitle, Widget? leading, Widget? child,
    List<Widget> actions = const [],
    double maxWidth = 720, double? maxHeight, double minHeight = 0,
    double headerSpacing = 16, double contentSpacing = 18, double actionsSpacing = 10,
    EdgeInsetsGeometry? padding,
    bool scrollable = true, ScrollController? scrollController,
    double? borderRadius, double borderWidth = 0.72,
    GlassStyle? style,
    GlassEffects? effects, // <- AJOUT 3
    List<Color>? customGradient, String? customKey,
    bool barrierDismissible = true, bool useSafeArea = true, bool useRootNavigator = true,
    ...
    return showDialog<T>(
      ...
      builder: (context) {
        Widget modal = UniversalGlassModal(
          ...
          title: title, subtitle: subtitle, leading: leading, child: child, actions: actions,
          maxWidth: maxWidth, maxHeight: maxHeight, minHeight: minHeight,
          headerSpacing: headerSpacing, contentSpacing: contentSpacing, actionsSpacing: actionsSpacing,
          padding: padding, scrollable: scrollable, scrollController: scrollController,
          borderRadius: borderRadius, borderWidth: borderWidth, style: style,
          effects: effects, // <- AJOUT 4
          customGradient: customGradient, customKey: customKey,
        );
  ...
  Widget _buildHeader(BuildContext context, GlassLayoutContext glass, bool isSmallMobile) {
    final hasTitle = (title?? '').trim().isNotEmpty;
    final hasSubtitle = (subtitle?? '').trim().isNotEmpty;
    if (!hasTitle &&!hasSubtitle && leading == null) return const SizedBox.shrink();
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final hasHeader = (title?? '').trim().isNotEmpty || (subtitle?? '').trim().isNotEmpty || leading!= null;
    ...
    final double effectiveMaxWidth = maxWidth < targetMaxWidth? maxWidth : targetMaxWidth;
    final double effectiveMaxHeight = maxHeight?? (screenHeight * 0.85);

    final EdgeInsetsGeometry effectivePadding = padding?? EdgeInsets.all(isSmallMobile? 14 : 20);
    final double effectiveBorderRadius = borderRadius?? (isSmallMobile? 20 : 28);

    final GlassStyle effectiveStyle = style?? glass.effectiveGlassStyle;
    final GlassEffects effectiveEffects = effects?? glass.effects; // <- FALLBACK SUR LE THEME
            ...
            child: GlassSurfaceContainer(
              ...
              child: Column(
                ...
                children: [
                  ...
                  if (!hasContent && hasHeader && hasActions) SizedBox(height: headerSpacing),
