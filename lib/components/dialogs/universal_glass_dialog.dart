...
class UniversalGlassDialog extends ConsumerWidget { // <- CONSUMER
  // ==========================================================================
  // HEADER
  // ==========================================================================
  final String? title;
      ...
      children: [
        ...
        if (hasSubtitle)
          Text(
            subtitle!,
            style: TextStyle(
              color: glass.palette.textSecondary.withValues(alpha: 0.68), // <- PALETTE GLASS
  ...
  Widget _buildActions(bool isSmallMobile) {
    ...
    if (isSmallMobile || actions.length > 2) { // <- auto column sur mobile
      return Column(
        mainAxisSize: MainAxisSize.min,
  ...
  Widget build(BuildContext context, WidgetRef ref) { // <- WIDGETREF
    final glass = ref.watchGlassContext(context); // <- CONTEXTE GLASS
    ...
    final double effectiveBorderRadius = borderRadius?? (isSmallMobile? 20 : glass.theme.borderRadius);
    ...
    return Material(
      ...
      child: Center(
        child: ConstrainedBox(
          ...
          child: GlassSurfaceContainer( // <- REMPLACE Container PAR GLASS
